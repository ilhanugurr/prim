<?php
/**
 * Primew Panel - Kategori Ekleme
 * Yeni envanter kategorisi ekleme sayfası
 */
...
// İzin kontrolü
if (!hasPagePermission('envanter', 'ekleme')) {
...
// Form gönderildi mi?
if ($_POST && isset($_POST['action']) && $_POST['action'] == 'add_kategori') {
    $data = [
    ...
    // Kategori adı benzersiz mi kontrol et
    if (empty($errors)) {
        $existing = $db->select('envanter_kategoriler', ['kategori_adi' => $data['kategori_adi']]);
        if (!empty($existing)) {
    ...
    if (empty($errors)) {
        if ($db->insert('envanter_kategoriler', $data)) {
...
<head>
    ...
    <title>SeoMEW Prim Sistemi - Kategori Ekle</title>
        ...
        <div class="main-content">
            ...
            <div class="content-area">
                ...
                <div style="background: var(--bg-card); border-radius: 12px; padding: 30px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08); border: 1px solid var(--border-color); margin-bottom: 30px;">
                    ...
                    <form method="POST" action="envanter-kategori-ekle.php">
