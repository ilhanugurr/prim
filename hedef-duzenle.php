...
if ($_POST && isset($_POST['action']) && $_POST['action'] == 'update_hedef') {
    ...
    if ($personel_id && !empty($firma_hedefleri)) {
        try {
            ...
            $error_message = "Hedef güncellenirken bir hata oluştu: " . $e->getMessage();
        }
    } else {
...
$hedefler = $db->query("
    ...
    WHERE h.personel_id = ? AND h.yil = ? AND h.ay = ?
", [$personel_id, $yil, $ay]);
...
foreach ($hedefler as $hedef) {
    $mevcut_hedefler[$hedef['firma_id']] = $hedef['aylik_hedef'];
...
<head>
    ...
    <title>SeoMEW Prim Sistemi - Hedef Düzenle</title>
        ...
        <div class="main-content">
            ...
            <div class="content-area">
                ...
                <div class="form-container">
                    <h2 class="form-title">Hedef Düzenle</h2>
                    
                    <form method="POST" action="hedef-duzenle.php">
                        <input type="hidden" name="action" value="update_hedef">
                        <input type="hidden" name="id" value="<?php echo $hedef_key; ?>">
                            ...
                            <select name="personel_id" class="form-select" required>
                                ...
                                <?php foreach ($personeller as $personel): ?>
                                    <option value="<?php echo $personel['id']; ?>" 
                                            <?php echo ($personel['id'] == $personel_id) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <div class="yil-tabs">
                                <?php for ($i = date('Y') - 1; $i <= date('Y') + 2; $i++): ?>
                                    <button type="button" class="yil-tab <?php echo ($i == $yil) ? 'active' : ''; ?>" 
                                            onclick="selectYil(<?php echo $i; ?>)">
                                        <?php echo $i; ?>
                                    </button>
                                <?php endfor; ?>
                            </div>
                            <input type="hidden" name="yil" id="selected-yil" value="<?php echo $yil; ?>">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="ay-tabs">
                                ...
                                ?>
                                    <button type="button" class="ay-tab <?php echo ($ay_no == $ay) ? 'active' : ''; ?>" 
                                            onclick="selectAy(<?php echo $ay_no; ?>)">
                                        <?php echo $ay_adi; ?>
                                    </button>
                                <?php endforeach; ?>
                            </div>
                            <input type="hidden" name="ay" id="selected-ay" value="<?php echo $ay; ?>">
                        </div>
                                ...
                                <div class="firma-hedef-item">
                                    <div class="firma-header">
                                        ...
                                        <input type="number" 
                                               ...
                                               value="<?php echo $mevcut_hedefler[$firma['id']] ?? ''; ?>"
                        ...
                        <div class="form-actions">
                            ...
                            <button type="submit" class="btn btn-primary">Hedef Güncelle</button>
                        </div>
                    </form>
                </div>
            </div>
